...
namespace Engine::Physics
{
    ...
    {
        return sqrtf(powf(component.Center.x - Center.x, 2) + powf(component.Center.y - Center.y, 2)) <= component.Radius + Radius;
    }
    ...
    bool Component<Circle>::Collide(Component<Quadrangle> const& component) const
    {
        ...
        {
            min = Point(min.x - Radius, min.y - Radius);
            max = Point(max.x + Radius, max.y + Radius);

            return (min.x <= Center.x and Center.x <= max.x) and (min.y <= Center.y and Center.y <= max.y);
        }
        else
        {
