...
namespace Rect
{
	namespace
	{
		...
		double const diagonal_theta = 45;
	...
	{//사각형 생성
		...
		rect_ang.push_back(new double[3]{});
	...
	void move_rect()
	{
		...
		for (size_t i = 0; i < rect_pos.size(); i++)
		{
			if (rect_ang[i][2] != 1)
			{
				...
				float costheta = static_cast<float>(cos((rect_ang[i][1]+ diagonal_theta) * radian));
				float sintheta = static_cast<float>(sin((rect_ang[i][1]+ diagonal_theta) * radian));

				//정사각형 rect의 각 꼭지점까지의 거리
				LONG size = static_cast<long>(halfwidth_size * sqrt(2));

				//각 점의 회전
				rect_pos[i][1].x = centerx + static_cast<long>(( size *  costheta));  //a점
				rect_pos[i][1].y = centery + static_cast<long>(( size *  sintheta));  //a점
				rect_pos[i][2].x = centerx + static_cast<long>(( size * -sintheta));  //b점
				rect_pos[i][2].y = centery + static_cast<long>(( size *  costheta));  //b점
				rect_pos[i][3].x = centerx + static_cast<long>(( size * -costheta));  //c점
				rect_pos[i][3].y = centery + static_cast<long>(( size * -sintheta));  //c점
				rect_pos[i][4].x = centerx + static_cast<long>(( size *  sintheta));  //d점
				rect_pos[i][4].y = centery + static_cast<long>(( size * -costheta));  //d점
			}
		}
	}
