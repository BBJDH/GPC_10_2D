...
bool Collide(struct_Rectangle const& LHS, struct_Rectangle const& RHS)
{
    ...
    vector<2> const axes[4] //axis의 복수형 : 중심선
    {
        {cos(LHS.Angle * radian),sin(LHS.Angle * radian)}, //LHS의 right벡터
        {-sin(LHS.Angle * radian),cos(LHS.Angle * radian)}, //LHS의 Up 벡터
        {cos(RHS.Angle * radian),sin(RHS.Angle * radian)}, //RHS의 right벡터
        {-sin(RHS.Angle * radian),cos(RHS.Angle * radian)}  //RHS의 Up 벡터
    };
