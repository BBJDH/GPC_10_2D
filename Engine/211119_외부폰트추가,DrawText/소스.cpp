...

    {
        ...
        while (true)
        {
            ...
            else
            {
                ...
                LPCSTR const lyric ="&&동해물과 백두산이 마르고 &닳도록"
                ...
                DrawText(hDC, lyric, ~'\0', &area,DT_CALCRECT| DT_WORDBREAK | DT_NOFULLWIDTHCHARBREAK |  DT_NOPREFIX);

                DrawText(hDC,lyric,~'\0',&area,DT_WORDBREAK|DT_NOFULLWIDTHCHARBREAK|DT_NOPREFIX); 
                //글자 단위 개행, 단어단위 개행, 잘리는 줄 버림, 접두사를 사용하지 않음(기호 그대로 출력)
